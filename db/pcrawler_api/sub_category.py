...
class sub_category(db_check):


    """
        \param id **bigint** *id of the location  使用范围：后端*
        \param name **text** *扩展分类名  name of the location*
        \param level **bigint** *define level or order  使用范围：后端*
        \param location_code **text** *internal usage only  使用范围：后端*
        \param link **text** *参数  use to define relation, like url or abbreviation*
        \param index_order **bigint** *排序权重  to control the show order*
        \param status **smallint** *状态  link-content: status[LEFT] status_name*
    """

    cache = list() #cache content of table

    _table_name='table_sub_category'
    ...
    _table_Kommentar = {"table_sub_category" : "", "id" : "id of the location\n\n使用范围：后端", "name" : "扩展分类名\n\nname of the location", "level" : "define level or order\n\n使用范围：后端", "location_code" : "internal usage only\n\n使用范围：后端", "link" : "参数\n\nuse to define relation, like url or abbreviation\n", "index_order" : "排序权重\n\nto control the show order", "status" : "状态\n\nlink-content: status[LEFT] status_name", };
    _table_atributes = {};
    _table_default_values = {"id" : "nextval('table_sub_category_id_seq'::regclass)", };
    ...
    def add(self, name, level=None, location_code=None, link=None, index_order=None, status=None):
        ...
        return self.DB_connection.insert(self._table_name, 'table_sub_category_id_seq', _parent_self=self, name=name, level=level, location_code=location_code, link=link, index_order=index_order, status=status)
